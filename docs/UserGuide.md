...
5. Square brackets to indicate a command is optional is not displayed in suggestion feature. Users need to refer to HelpGuide or UserGuide. This extra `[ ]`, if added in suggestion (e.g. `[e/EMAIL]`), will mess with parsing of Autocomplete feature. (`[ ]` brackets in general do not affect parsing).
